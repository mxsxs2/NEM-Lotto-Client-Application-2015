...
public class openBTCWallet extends Entries{
    ...
    private boolean getAddresses(){        //get the active addresses and ballances from the wallet (it gets in bult rather then multiple queries)
        try {
            utils.WalletEncrypt WE=new utils.WalletEncrypt();
            utils.URLtoJSON URLtoJSON=new utils.URLtoJSON();
            JSONObject json = URLtoJSON.open(this.URL+"merchant/"+this.GUID+"/list?password="+WE.pswd(this.Creator, this.Time)+"&api_code="+this.ApiCode);
